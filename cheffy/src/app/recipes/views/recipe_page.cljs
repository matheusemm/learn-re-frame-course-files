...
(defn recipe-page
  ...
  (let [{:keys [name ]} @(rf/subscribe [:recipe])
    ...
    [:> Box
     [page-nav {:left (if (= active-nav :saved) :saved :recipes)
                :center (if author? [recipe-editor] name)
                :right (cond
                         (not logged-in?)
                         [:> Button {:as "a"
                                     :color "white"
     ...
     [:> Box
      [:> Row
       [:> Col {:xs 12 :sm 6}
        [:> Box {:pb 20} [recipe-info]]
        [:> Box {:pb 20} [recipe-image]]
        [:> Box {:pb 20} [recipe-ingredients]]]
       [:> Col {:xs 12 :sm 6}
        [:> Box {:pb 20} [recipe-steps]]]]]]))
